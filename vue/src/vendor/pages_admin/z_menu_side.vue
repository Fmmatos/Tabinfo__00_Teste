...
import { html_treatment, open__ } from '@/vendor/services/events';
...
import { inject } from 'vue';
    ...
    // FUNCTIONS
        const __active = (value: any) => {
            if (value?.pages && (value.pages.includes($_GET[2]) || value.pages.includes(Number($_GET[2])) )){
                return true;
            }
            if(value?.url == `/` && !$_GET[2]){
                return true;
        ...
        const __open = (value: any) => {
            ...
            if (value?.url){
                if (value?.table){
                    open__(value.url, { n: value.table, type_items: value.type_items, init__: 1 });
                } else {
                    open__(value.url, { init__: 1 });
                }
            }
            return null;
        }
        ...
        const __active_open_submenu = (value: any) => {
            if (__active(value) || SHOW.submenu){
                return true;
            }
...
<template>
    <div class="posa t0 l0 z9 mt0"> <a :href="`${DIR()}/admin/menu_admin/?init__=1&t=0`" class="db w15 h10"></a> </div>

    <!-- MENU -->
        <div v-if="OBJ?.menu_side" class="posr z5 bg_EBEBEB" :class="`w${SHOW.MENU_SIDE__WIDTH}`" style="transition: margin .5s" :style="`${SHOW.MENU_SIDE ? `` : `margin-left: -${SHOW.MENU_SIDE__WIDTH}px;`}`">
            <div class="posf t0 z1 h100p bg_EBEBEB" :class="`w${SHOW.MENU_SIDE__WIDTH}`"></div>

            <ul class="posr z2 pt12 pl10 pr10 mb40 menu">
                <li v-for="(value, key) in OBJ.menu_side" :key="key" class="posr">
                    <a @click="__open(value)" class="h28 pl10 pr10 fwb5 flexx flex_ac bg_hover_ffffff5F br6" :class="`${__active(value) ? `active` : ``} ${value.svg_class}`">
                        <!-- ICON -->
                            <div class="w20 flexx flex_c flex_ac c_444">
                                <div v-if="value?.svg">
                                    <div v-if="__active(value) && value?.svg_active" class="w16 flexx flex_c flex_ac" v-html="html_treatment(value.svg_active)"></div>
                                    <div v-else class="w16 flexx flex_c flex_ac" v-html="html_treatment(value.svg)"></div>
                                </div>
                                <div v-else-if="value?.icon">
                                    <div v-if="__active(value) && value?.icon_active"><i class="fz15" :class="value.icon_active"></i></div>
                                    <div v-else><i class="fz15" :class="value.icon"></i></div>
                                </div>
                            </div>
                        <!-- ICON -->
                        <div class="pl8 fz13">{{ value.name }}</div>
                    </a>

                    <ul v-if="__active_open_submenu(value)" class="submenu pb2">
                        <li v-for="(value_1, key_1) in value.submenu" :key="key_1" class="posr">
                            <a @click="__open(value_1)" class="min-h20 p5 pl10 pr10 flexx flex_ac c_444 br6" :class="`${value_1?.class ? value_1.class : ``} ${__active(value_1) ? `active` : ``}`">
                                <div class="w20"></div>
                                <div class="pl8 fz13">{{ value_1.name }}</div>
                            </a>
                        </li>
                    </ul>
                </li>
    ...
    <!-- BACKGROUND -->
        <div v-if="SHOW.MENU_SIDE" @click="SHOW.MENU_SIDE = !SHOW.MENU_SIDE" class="posf t0 l0 z4 w100p h100p bg_0000009F dnn_1400"></div>
    <!-- BACKGROUND -->
...
<style>
    .__MENU_SIDE__ svg { fill: #444 !important; }
