...
import { tooltip, split, select2, select2_reset, value__ } from '@/vendor/services/events';
...
<template>
    ...
    <slot>
        <div class="__input_all__ db __INPUT__SELECT_CITY__" :class="`__INPUT__NAME__${PROPS.name}__`">
            <div>
                ...
                <div class="__input__ posr">
                    ...
                    <select :name="name" v-model="FORM.v[PROPS.name]" :id="`select_${SELECT.select_id}`" v-bind="tags(PROPS, `select`).attributes" v-on="tags(PROPS, `select`, false).events">
                        ...
                        <option v-for="(value_1, key_1) in OBJ.CITY" :key="key_1" :value="value_1.name">{{ value_1.name }}</option>
                    </select>
                </div>
            </div>
        </div>
